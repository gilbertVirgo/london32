...
export default function CollaborateAnimation() {
	...
	const draw = useCallback(() => {
		...
		for (let i = 0; i < 24; i++) {
			...
			ctx.fillStyle = i % 2 === 0 ? "#fdca40" : "#df2935"; // Brand yellow on even, brand red on odd
			ctx.beginPath();
		...
		const tick = () => {
			...
			for (let i = 0; i < 24; i++) {
				...
				if (phaseRef.current === "in") {
					target = isYellow
						? (i / 2 < currentStepRef.current ? 1.0 : 0.0)
						: ((23 - i) / 2 < currentStepRef.current ? 1.0 : 0.0);
				} else if (phaseRef.current === "out") {
					target = isYellow
						? (i / 2 < currentStepRef.current ? 0.0 : 1.0)
						: ((23 - i) / 2 < currentStepRef.current ? 0.0 : 1.0);
				}
